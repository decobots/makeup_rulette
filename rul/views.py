...
def random_selector(request):
    palettes_id = users_palettes(request.user.pk)
    shades = Shade.objects.filter(palette_id__in=palettes_id).select_related('palette__seller').all().order_by('?')[:4]
...
def insta_glam_selector(request):
    ...
    crease = Shade.objects.select_related('palette__seller').filter(
        texture='M', darkness=3, palette_id__in=palettes_id).all().order_by('?')[:1]

    inner = Shade.objects.select_related('palette__seller').filter(
        palette_id__in=palettes_id).filter(
    ...
    outer_v = Shade.objects.select_related('palette__seller').filter(
        palette_id__in=palettes_id, darkness__lt=3).all().order_by('?')[:1]

    blending = Shade.objects.select_related('palette__seller').filter(
        palette_id__in=palettes_id, darkness__gt=3).all().order_by('?')[:1]
...
def users_palettes(user_id):
    user_palettes = UserPalette.objects.select_related('palette__seller').filter(user_id=user_id).all()
    return [p.palette.id for p in user_palettes]
