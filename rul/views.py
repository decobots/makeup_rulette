...
def index(request):
    return HttpResponse("Hello, world. You're at the rul index.")
...
def insta_glam_selector(request):
    ...
    inner = Shade.objects.filter(Q(texture='Sh') | Q(texture='Sp') | Q(texture='G') | Q(darkness__gt=3)).all().order_by('?')[:1]
